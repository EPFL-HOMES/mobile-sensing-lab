...
export function ShiftGroupsEditor({ value, onChange }: { value: SupplyEditor; onChange: (patch: Partial<SupplyEditor>) => void }) {
  const fleetSize = value.fleet_size ?? 1;
  return <Stack gap={2}>
    ...
    {!!value.shift_groups?.length && <>
      <Typography color="text.secondary">Group shares must sum to 100%. Shares are converted to integer vehicle counts against the fixed fleet size ({fleetSize}); choose percentages that produce an exact partition. Each vehicle has one shift. Earlier-day starts create explicit carry-in vehicles; add warm-up demand in Simulation when their pre-day activity is needed.</Typography>
      {value.shift_groups.map((row, i) => <Stack gap={1} key={i} sx={{ p: 2, border: "1px solid", borderColor: "divider", borderRadius: 1 }}>
        <div className="form-grid">
          <TextField size="small" label="Shift name" value={row.name} onChange={(e) => onChange({ shift_groups: value.shift_groups?.map((old, j) => i === j ? { ...old, name: e.target.value } : old) })} />
          <TextField size="small" label="Share of fleet (%)" type="number" inputProps={{ min: 100 / fleetSize, max: 100, step: 1 }} value={100 * row.count / fleetSize} helperText={`${row.count} of ${fleetSize} vehicles`} onChange={(e) => onChange({ shift_groups: value.shift_groups?.map((old, j) => i === j ? { ...old, count: Math.max(1, Math.round(Number(e.target.value) * fleetSize / 100)) } : old) })} />
          {(["start_time", "latest_start", "work_hours"] as const).map((field) => <TextField key={field} size="small" label={{ start_time: "Earliest start", latest_start: "Latest start", work_hours: "Work duration (hours)" }[field]} type={field === "work_hours" ? "number" : "text"} value={row[field]} onChange={(e) => onChange({ shift_groups: value.shift_groups?.map((old, j) => i === j ? { ...old, [field]: field === "work_hours" ? Number(e.target.value) : e.target.value } : old) })} />)}
