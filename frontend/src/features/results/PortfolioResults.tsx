...
import type { PortfolioBudgetView, PortfolioFrontierPoint, PortfolioFrontierView } from "../../api/generated";
import { requestJson } from "../../api/client";
import { ScientificChart } from "../../components/ScientificChart";
import { ScientificMap } from "../../components/ScientificMap";
import { ExportActions } from "./ExportActions";
import { portfolioColors as colors, portfolioTraces } from "./portfolioPresentation";
...
function PortfolioInspector({ point, unit }: { point: PortfolioFrontierPoint; unit: string }) {
  return <Stack gap={2.5}>
    ...
    <Typography variant="caption" color="text.secondary">Display rounded to five decimals. Analysis and exports retain full precision. Std describes empirical variability, not a confidence interval.</Typography>
  </Stack>;
}
...
export function PortfolioResults({ jobs, revisionForJob, preferredView }: { jobs: JobSnapshot[]; revisionForJob: (jobId: string) => string | null; preferredView?: string }) {
  ...
  const frontiers = useQueries({ queries: budgetRows.map(budget => ({ queryKey: ["portfolio-frontier", resourceId, budget.budget_id], queryFn: () => requestJson<PortfolioFrontierView>(`/api/v1/portfolio-frontiers/${resourceId}?budget_id=${encodeURIComponent(budget.budget_id)}`), enabled: Boolean(resourceId) })) });
  const allPoints = frontiers.flatMap(query => query.data?.points ?? []);
  ...
  const portfolioId = requestedPortfolio && (!frontiersSettled || uniquePoints.some(point => point.portfolio_id === requestedPortfolio)) ? requestedPortfolio : uniquePoints[0]?.portfolio_id ?? "";
  ...
  return <Stack gap={2.5}>
    ...
    <Box className="portfolio-overview">
      <Paper variant="outlined" className="analysis-panel"><Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={2} flexWrap="wrap"><Box><Typography variant="h6">Utility frontier</Typography><Typography variant="body2" color="text.secondary">Compare all configured budgets. Select a point to inspect its sensor counts, costs and mean sensing coverage.</Typography></Box><FormControlLabel control={<Checkbox checked={nondominatedOnly} onChange={event => set("nondominated_only", event.target.checked ? "" : "0")} />} label="Show nondominated points only" /></Stack>
        ...
        <Typography variant="caption" color="text.secondary">{p05Risk ? "Mean and P05 are both maximized. P05 is an empirical quantile, not a guaranteed minimum. " : "Mean is maximized; standard deviation is minimized. "}Each combination is drawn once. Color identifies its lowest feasible budget; muted circles are dominated and solid diamonds are nondominated. The outlined point is selected.</Typography>
      </Paper>
