...
def main():
    ...
    config = LausanneSmokeConfig.model_validate_json(
        (repo / "docs/templates/simulation/lausanne_smoke.json").read_bytes()
    )
    environment = None
    warm = None
    if args.warm_report:
    ...
    value = json.loads((repo / "docs/templates/portfolio/lausanne_counts.json").read_text())
    ...
    weights = UtilityWeightResource.model_validate_json(
        (repo / "docs/templates/portfolio/lausanne_uniform_weights.json").read_bytes()
    )
    started = time.perf_counter()
